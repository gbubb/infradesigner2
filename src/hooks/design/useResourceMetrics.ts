...
export const useResourceMetrics = () => {
  ...
  const resourceMetrics = useMemo(() => {
    ...
    if (!activeDesign || !activeDesign.components || !Array.isArray(activeDesign.components) || activeDesign.components.length === 0) {
      return metrics;
    }
    ...
    try {
      ...
      metrics.totalRackQuantity = Math.ceil(metrics.totalRackUnits / (rackUnitsPerRack || 42));
      ...
      if (coloRacks) {
        metrics.monthlyColoCost = metrics.totalRackQuantity * (rackCostPerMonth || 0);
      }
      ...
      const maxRackUnits = metrics.totalRackQuantity * (rackUnitsPerRack || 42);
